...
const _cellSize = 30.0;
const _numColumns = 10;
...
class GridColorSelector extends StatelessWidget {
  ...
  const GridColorSelector({Key key, this.selectedColor, this.onColorSelected, this.onEyePick}) : super(key: key);
  ...
  List<Widget> _buildColorsTiles() => _buildColorGridValues().map(_buildCell).toList();
  ...
  bool isSelected(Color color) => selectedColor?.withOpacity(1)?.value == color.withOpacity(1).value;
}
